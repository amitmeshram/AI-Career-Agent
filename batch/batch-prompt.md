...
Sección de **gaps** con esta estructura exacta:

| Gap | Hard/Soft | Adjacent Experience | Mitigation |
|---|---|---|---|

- Los pointers incluidos bajo `Major:` en `main_gap` deben categorizarse como `Hard`.
- Los pointers incluidos bajo `Minor:` en `main_gap` deben categorizarse como `Soft`.
- Cada fila debe usar únicamente `Hard` o `Soft` en la segunda columna.
...
Before scoring, read `modes/_profile.md`. If it contains an
`Evaluation Scorecard Override`, use those dimensions, weights, calculation,
and safeguards instead of the default scorecard.

For the active scorecard, include:

| Dimension | Weight | Score | Weighted Contribution | Evidence Basis |
|-----------|--------|-------|-----------------------|----------------|
| {active dimension} | {weight} | X/5 | X.XX | ... |
| **Global Score** | **100%** |  | **X/5** | Rounded as configured. |
...
### Paso 4 — Generar Executive Report DOCX inmediatamente

Tan pronto como se escriba este report `.md`, ejecutar el siguiente comando
antes de generar el PDF, escribir el tracker o finalizar el worker:
...
No diferir este paso hasta que termine el batch. Cada worker genera el DOCX
de su report inmediatamente después de guardar su `.md`. Si falla, continuar
el resto del pipeline y devolver el error del DOCX en el resultado.
...
```json
{
  ...
  "role": "{rol_o_unknown}",
  "score": null,
  "executive_docx": "{ruta_docx_si_existe}",
  "pdf": null,
