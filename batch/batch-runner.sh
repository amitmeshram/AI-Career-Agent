...
process_offer() {
  ...
  if [[ $exit_code -eq 0 ]]; then
    # The worker must generate the executive DOCX immediately after its report.
    # This fallback repairs older workers before the offer is marked complete.
    local report_file
    report_file=$(find "$REPORTS_DIR" -maxdepth 1 -type f -name "${report_num}-*.md" -print -quit 2>/dev/null || true)
    if [[ -n "$report_file" ]]; then
      # Regenerate idempotently because the role-based output filename cannot be
      # derived reliably from the report filename alone.
      python "$PROJECT_DIR/tools/generate_executive_report_from_evaluation.py" \
        --report "$report_file" \
        --cv "$PROJECT_DIR/cv.md" \
        --force \
        >> "$log_file" 2>&1 || true
    fi
