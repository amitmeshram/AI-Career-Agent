...
| `reports/` | Evaluation reports (format: `{###}-{company-slug}-{YYYY-MM-DD}.md`). Executive Word reports are stored in `data/cv_optimization/reports/{company-slug}-{role-slug}-executive-report.docx`. Blocks A-F + G (Posting Legitimacy). Header includes `**Legitimacy:** {tier}`. |
...
- **RULE: Immediately after writing each evaluation `.md`, generate its executive DOCX with `python tools/generate_executive_report_from_evaluation.py --report <report.md> --cv cv.md --force`. Output naming is `{company-slug}-{role-slug}-executive-report.docx` under `data/cv_optimization/reports/`. Do not defer DOCX generation until all reports finish.**
