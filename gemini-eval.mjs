 ...
 *
 * Requires:
 *   AI_API_KEY with AI_PROVIDER_NAME=gemini in .env (or GEMINI_API_KEY)
 *
...
function generateExecutiveReport(reportPath) {
  const scriptPath = join(ROOT, 'tools', 'generate_executive_report_from_evaluation.py');
  const cvPath = CV_PATH;
  ...
  for (const command of candidates) {
    const commandArgs = command === 'py'
      ? ['-3', scriptPath, '--report', reportPath, '--cv', cvPath, '--force']
      : [scriptPath, '--report', reportPath, '--cv', cvPath, '--force'];
    ...
    if (result.status === 0) {
      const match = (result.stdout || '').match(/Executive DOCX path:\s*(.+)/);
      const outputPath = match ? match[1].trim() : 'data/cv_optimization/reports/';
      console.log(`✅  Executive DOCX saved: ${outputPath.replaceAll('\\', '/')}`);
      return outputPath;
    }
  ...
  throw new Error(lastError || 'No usable Python runtime found.');
}
...
╚══════════════════════════════════════════════════════════════════╝
  ...
  SETUP
    ...
    2. Add AI_PROVIDER_NAME=gemini and AI_API_KEY=<your-key> to .env
...
// ---------------------------------------------------------------------------
const apiKey = (
  String(process.env.AI_PROVIDER_NAME || '').trim().toLowerCase() === 'gemini'
    ? process.env.AI_API_KEY
    : ''
) || process.env.GEMINI_API_KEY;
if (!apiKey) {
  console.error(`
❌  AI_API_KEY not found for Gemini.

   1. Get a free key at https://aistudio.google.com/apikey
   2. Add it to .env:   AI_PROVIDER_NAME=gemini and AI_API_KEY=your_key_here
...
try {
  ...
  if (err.message?.includes('API_KEY')) {
    console.error('    Check AI_PROVIDER_NAME=gemini and AI_API_KEY in .env');
...
`;
    ...
    try {
      generateExecutiveReport(reportPath);
    } catch (docxError) {
      console.warn(`⚠️   Could not generate executive DOCX: ${docxError.message}`);
    }
