...
14. Escribe HTML a `output/tmp/cv-{candidate}-{company}.html` (crear `output/tmp/` si no existe; funciona en Windows y macOS)
15. Ejecuta: `node generate-pdf.mjs output/tmp/cv-{candidate}-{company}.html output/cv-{candidate}-{company}-{YYYY-MM-DD}.pdf --format={letter|a4}`
