...
2. Executive DOCX in `data/cv_optimization/reports/`, generated immediately after its `.md`
3. PDF in `output/`
4. Tracker line in `batch/tracker-additions/{id}.tsv`
5. Result JSON via stdout

The order is mandatory per worker:
`.md` → executive `.docx` → PDF → tracker. Workers must not defer DOCX
generation until the end of the batch.
