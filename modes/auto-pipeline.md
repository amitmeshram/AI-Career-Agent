...
Ejecutar exactamente igual que el modo `oferta` (leer `modes/oferta.md` para todos los bloques A-F + Block G Posting Legitimacy + Section H Missing Keywords).
...
Include Blocks G-H in the saved report. Add `**Legitimacy:** {tier}` to the report header.

## Paso 3 — Generar Executive Report DOCX inmediatamente

Inmediatamente después de guardar cada report `.md`, y antes del PDF, tracker
o siguiente oferta, ejecutar:
...
3. **Guardar en el report** como sección `## I) Draft Application Answers`.
